with seasons as (
    select * from {{ ref('stg_f1db__season') }}
),
...
driver_standings as (
    select * from {{ ref('stg_f1db__season_driver_standing') }}
),
...
constructor_counts as (
    select
        season,
        count(distinct constructor_id)::integer as constructor_count
    from {{ ref('int_f1db__race_results_with_entities') }}
    group by season
),
...
from seasons
left join race_counts
    on seasons.season = race_counts.season
left join driver_counts
    on seasons.season = driver_counts.season
left join constructor_counts
    on seasons.season = constructor_counts.season
left join wdc
    on seasons.season = wdc.season
left join wcc
    on seasons.season = wcc.season
