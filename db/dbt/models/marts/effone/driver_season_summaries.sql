...
select
    ...
    (coalesce(race_aggregates.race_points, 0) + coalesce(sprint_aggregates.sprint_points, 0)) - season_drivers.points as points_delta,
    (((coalesce(race_aggregates.race_points, 0) + coalesce(sprint_aggregates.sprint_points, 0)) - season_drivers.points) * 100)::integer as points_delta_x100,
    {{ var('refresh_id') }}::bigint as refresh_id
from season_drivers
join drivers
    on season_drivers.driver_id = drivers.driver_id
